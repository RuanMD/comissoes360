...
async function introspect(typeName) {
    ...
    if (data.data && data.data.__type) {
        data.data.__type.fields.forEach(f => {
            let typeInfo = f.type.name || f.type.ofType?.name || f.type.ofType?.ofType?.name || "Unknown";
            console.log(`- ${f.name} (${typeInfo})`);
        });
    } else {
...
async function run() {
    await introspect("ConversionOrder");
    await introspect("ConversionItem");
    await introspect("ConversionOrderItem"); // Common name variation
}
