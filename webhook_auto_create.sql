...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    ...
    -- Ou você pode modificar sua aplicação para login via Magic Link (apenas email)
    v_password := '123456';
    
    -- No Supabase, recomendamos usar a API de Admin (supabase-js) no N8N 
    -- em vez de tentar forçar um insert bruto no auth.users por conta dos hooks e salts.
    -- No entanto, como bypass temporário em RPC (Requer permissões especiais):
  ...
  -- 3. Atualiza os dados públicos (tanto para novos quanto para já existentes)
  UPDATE public.users 
  SET 
